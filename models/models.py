...
class UNetClassifier(nn.Module):
    ...
    def __init__(self, filters: int = 64, input_filters: int = 3, **kwargs):
        ...
        self.lin1 = nn.Linear(256 * 256, 128)
...
class UNet(nn.Module):
    ...
    def __init__(
        self, filters: int = 64, input_filters: int = 3, out_channels=1, **kwargs
    ):
        """
        Create U-Net model with:
...
class ConvAutoencoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            # conv 3
            nn.Conv2d(in_channels=32, out_channels=1, kernel_size=11),
        )

        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(in_channels=3, out_channels=32, kernel_size=11),
            # conv 9
            nn.ConvTranspose2d(
                in_channels=32,
                out_channels=16,
...
class UnFlatten(nn.Module):
    def forward(self, input, size=53824):
        return input.view(input.size(0), 16, 58, 58)


class ConvVAE(nn.Module):
    def __init__(self, device, h_dim=53824, z_dim=256):
        ...
        self.log_var = nn.Linear(self.h_dim, self.z_dim)
        self.fc = nn.Linear(self.z_dim, self.h_dim)

        self.decoder = nn.Sequential(
            UnFlatten(),
            nn.ConvTranspose2d(in_channels=16, out_channels=32, kernel_size=7),
            # conv 9
            nn.ConvTranspose2d(
                in_channels=32,
                out_channels=16,
...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(16 * 254 * 254, 1024)
